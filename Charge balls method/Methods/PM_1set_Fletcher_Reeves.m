function result = PM_1set_Fletcher_Reeves(fun , init) % Penalty method
    EPS = 1e-6;
    
    n = length(fun.c);
    x = zeros(n, 1);
    r = 10;
    x = Fletcher_Reeves(fun, init, 1);
...
function result = Fletcher_Reeves(fun, initialPoint, r)
    ...
    while norm(F_grad(fun, x, r)) > EPS
        ...
        if mod(iteration + 1, n) ~= 0
            b = (norm(F_grad(fun, nextX, r)) / norm(F_grad(fun, x, r)))^2;
        else
            b = 0;
        end
